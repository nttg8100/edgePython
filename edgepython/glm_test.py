...
def glm_treat(glmfit, coef=None, contrast=None, lfc=np.log2(1.2), null="interval"):
    ...
    # Offset adjustment
    offset = np.asarray(glmfit.get("offset", np.zeros((ngenes, design.shape[0]))), dtype=np.float64)
    if offset.ndim == 1:
    ...
    if null == "interval":
        ...
        p_value[j] = _integrate_pnorm(-z_right[j], -z_right[j] + c) + _integrate_pnorm(
            z_left[j] - c, z_left[j]
        )
    ...
    table = pd.DataFrame({"logFC": logFC, "logCPM": glmfit["AveLogCPM"], "PValue": p_value})
...
def _integrate_pnorm(a, b):
    ...
    result = np.where(
        equal,
        norm_dist.cdf(a),
        (b * norm_dist.cdf(b) + norm_dist.pdf(b) - (a * norm_dist.cdf(a) + norm_dist.pdf(a)))
        / np.maximum(b - a, 1e-15),
    )
    return result
