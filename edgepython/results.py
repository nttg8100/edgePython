...
def top_tags(obj, n=10, adjust_method="BH", sort_by="PValue", p_value=1.0, coef=None):
    ...
    if adjust_method != "none":
        ...
        if valid_mask.all():
            ...
            _, adj_pvals[valid_mask], _, _ = multipletests(raw_pvals[valid_mask], method=sm_method)
        else:
            adj_pvals = np.full_like(raw_pvals, np.nan)
