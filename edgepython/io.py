...
def read_dge(files, path=None, columns=(0, 1), group=None, labels=None, sep="\t"):
    ...
    return make_dgelist(counts, samples=samples, genes=pd.DataFrame({"GeneID": all_gene_ids}))
...
def feature_counts_to_dgelist(files):
    ...
    for f in files:
        ...
        count_cols = [
            c for c in df.columns if c not in ["Geneid", "Chr", "Start", "End", "Strand", "Length"]
        ]
...
def _estimate_overdispersion(overdisp, df_arr):
    ...
    if n_pos > 0:
        ...
        df_prior = 3.0
        overdisp_prior = float(np.median(overdisp[pos])) / f_dist.ppf(
            0.5, dfn=df_median, dfd=df_prior
        )
        if overdisp_prior < 1.0:
            overdisp_prior = 1.0

        overdisp[pos] = (df_prior * overdisp_prior + df_arr[pos] * overdisp[pos]) / (
            df_prior + df_arr[pos]
        )
...
def _read_kallisto_h5(paths, verbose):
    ...
    try:
        ...
        raise ImportError(
            "h5py package required for kallisto H5 format. Install with: pip install h5py"
        )
    ...
    for j, p in enumerate(paths):
        ...
        with h5py.File(h5_file, "r") as f:
            ...
            raw_ids = f["aux"]["ids"][:]
            ids = np.array([s.decode("utf-8") if isinstance(s, bytes) else str(s) for s in raw_ids])
...
def _read_rsem_data(files, path, ngibbs, verbose):
    ...
    for j, f in enumerate(files):
        ...
        if j == 0:
            ...
            lengths = (
                quant_df["length"].values.astype(np.int64) if "length" in quant_df.columns else None
            )
            eff_lengths = (
                quant_df["effective_length"].values.astype(np.float64)
...
def _read_anndata(data, group, labels, obs_col, layer, verbose):
    ...
    try:
        ...
        raise ImportError(
            "anndata package required for AnnData/.h5ad import. Install with: pip install anndata"
        )
    from .dgelist import make_dgelist
    ...
    if layer is not None:
        if layer not in adata.layers:
            raise ValueError(
                f"Layer '{layer}' not found in AnnData. Available: {list(adata.layers.keys())}"
            )
    ...
    if group is None and obs_col is not None:
        if obs_col in adata.obs.columns:
            ...
            raise ValueError(
                f"Column '{obs_col}' not found in AnnData.obs. Available: {list(adata.obs.columns)}"
            )
...
def _read_rds(filepath, group=None, verbose=True):
    ...
    try:
        ...
        samples_df = (
            pd.read_csv(samples_path, index_col=0) if os.path.exists(samples_path) else None
        )
...
def _read_table_file(data, path, columns, sep, group, verbose):
    ...
    return read_dge(files, path=path, columns=columns or (0, 1), group=group, sep=sep)
    ...
    *,
    ...
    if source == "anndata":
        return _read_anndata(
            data, group=group, labels=labels, obs_col=obs_col, layer=layer, verbose=verbose
        )
    ...
    if source == "table":
        return _read_table_file(
            data, path=path, columns=columns, sep=sep, group=group, verbose=verbose
        )
...
def read_bismark2dge(files, sample_names=None, verbose=True):
    ...
    for i in range(nsamples):
        chr_ints = np.array([chr_to_int[c] for c in chr_rle_list[i]], dtype=np.float64)
    ...
    # Gene annotation
    genes_df = pd.DataFrame({"Chr": chr_arr, "Locus": locus_arr}, index=row_names)
...
def seurat_to_pb(object, sample, cluster="cluster"):
    ...
    if _anndata_cls is not None and isinstance(object, _anndata_cls):
        ...
        raise TypeError(f"Expected AnnData, dict, or ndarray, got {type(object).__name__}")
    ...
    if isinstance(sample, str):
        if obs is None:
            raise ValueError("sample is a column name but no obs/metadata provided")
        if sample not in obs.columns:
            raise ValueError(f"Column '{sample}' not found in obs. Available: {list(obs.columns)}")
        sample_labels = obs[sample].values
    else:
        sample_labels = np.asarray(sample)
        if len(sample_labels) != n_cells:
            raise ValueError(f"sample length ({len(sample_labels)}) != number of cells ({n_cells})")

    if isinstance(cluster, str):
        if obs is None:
            raise ValueError("cluster is a column name but no obs/metadata provided")
        if cluster not in obs.columns:
            raise ValueError(f"Column '{cluster}' not found in obs. Available: {list(obs.columns)}")
        cluster_labels = obs[cluster].values
    else:
        cluster_labels = np.asarray(cluster)
        if len(cluster_labels) != n_cells:
            raise ValueError(
                f"cluster length ({len(cluster_labels)}) != number of cells ({n_cells})"
            )
    ...
    combined = np.array([f"{s}_cluster{c}" for s, c in zip(sample_labels, cluster_labels)])
...
def to_anndata(obj, adata=None):
    ...
    try:
        ...
        raise ImportError(
            "anndata package required for AnnData export. Install with: pip install anndata"
        )
