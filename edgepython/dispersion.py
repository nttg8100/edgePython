...
from .dispersion_lowlevel import (
    adjusted_profile_lik,
    adjusted_profile_lik_grid,
    maximize_interpolant,
    cond_log_lik_der_delta,
    common_cond_log_lik_der_delta,
    _cond_log_lik_grid,
    disp_cox_reid,
    disp_cox_reid_interpolate_tagwise,
    disp_cox_reid_spline_trend,
    disp_cox_reid_power_trend,
    disp_bin_trend,
    disp_pearson,
    disp_deviance,
)
...
def estimate_disp(
    ...
    if design is None:
        ...
        if np.all(
            np.bincount(
                group.astype(int)
                if np.issubdtype(group.dtype, np.integer)
                else np.searchsorted(unique_groups, group)
            )
            <= 1
        ):
            warnings.warn("There is no replication, setting dispersion to NA.")
            return {
                "common.dispersion": np.nan,
        ...
        l0 = adjusted_profile_lik_grid(spline_disp, sely, design, seloffset, weights=selweights)
    ...
    if trend_method != "none":
        ave_lcpm = ave_log_cpm(y, lib_size=lib_size, dispersion=common_dispersion, weights=weights)
        out_1 = WLEB(
            theta=spline_pts,
            loglik=l0,
            covariate=ave_lcpm[sel],
            trend_method=trend_method,
            span=span,
            legacy_span=legacy_span,
            overall=False,
    ...
    if not tagwise:
        return {"common.dispersion": common_dispersion, "trended.dispersion": trended_dispersion}
    ...
    if prior_df is None:
        ...
        s2_fit = squeeze_var(
            s2, df=df_residual, covariate=covariate, robust=robust, winsor_tail_p=winsor_tail_p
        )
...
def WLEB(
    ...
    # Overall prior
    if overall:
        out["overall"] = maximize_interpolant(theta, np.sum(loglik, axis=0).reshape(1, -1))[0]
...
def estimate_common_disp(y, group=None, lib_size=None, tol=1e-6, rowsum_filter=5, verbose=False):
    ...
    if isinstance(y, dict) and "counts" in y:
        ...
        d = estimate_common_disp(
            dge["counts"],
            group=group,
            lib_size=ls,
            tol=tol,
            rowsum_filter=rowsum_filter,
            verbose=verbose,
        )
...
def estimate_tagwise_disp(
    ...
    out = WLEB(
        ...
        if trend == "movingave"
        else ("loess" if trend == "loess" else "none"),
        span=span,
    )
...
def estimate_trended_disp(
    y, group=None, lib_size=None, ave_log_cpm_vals=None, method="bin.spline", df=5, span=2 / 3
):
    ...
    bins = cut_with_min_n(ave_log_cpm_vals, intervals=nbins, min_n=max(1, ntags // nbins))
    ...
    for i in range(1, nbins + 1):
        ...
        if np.sum(mask) == 0:
            continue
        disp_bins[i - 1] = estimate_common_disp(
            y[mask], group=group, lib_size=lib_size, rowsum_filter=0
        )
    ...
    if method == "bin.spline":
        ...
        try:
            f = interp1d(ave_bins, np.sqrt(np.maximum(disp_bins, 0)), fill_value="extrapolate")
...
def estimate_glm_common_disp(
    ...
    if method == "CoxReid":
        ...
        d = disp_pearson(
            y, design=design, offset=offset, subset=subset, ave_log_cpm_vals=ave_log_cpm_vals
        )
    else:
        d = disp_deviance(
            y, design=design, offset=offset, subset=subset, ave_log_cpm_vals=ave_log_cpm_vals
...
def estimate_glm_trended_disp(
    y, design=None, offset=None, ave_log_cpm_vals=None, method="auto", weights=None
):
    ...
    if method in ("bin.spline", "bin.loess"):
        ...
        result = disp_cox_reid_power_trend(
            y, design, offset=offset, ave_log_cpm_vals=ave_log_cpm_vals
        )
    else:
        result = disp_cox_reid_spline_trend(
            y, design, offset=offset, ave_log_cpm_vals=ave_log_cpm_vals
        )
...
def estimate_glm_tagwise_disp(
    ...
    if isinstance(y, dict) and "counts" in y:
        ...
        if trend:
            ...
            if dispersion is None:
                raise ValueError(
                    "No trended.dispersion found. Run estimate_glm_trended_disp first."
                )
        else:
            if dispersion is None:
                dispersion = dge.get("common.dispersion")
                if dispersion is None:
                    raise ValueError(
                        "No common.dispersion found. Run estimate_glm_common_disp first."
                    )
...
def _calc_resid(fit, residual_type="pearson"):
    ...
    if residual_type == "pearson":
        ...
        with np.errstate(divide="ignore", invalid="ignore"):
            r = 2 * (
                y_adj * np.log(np.maximum(y_adj, 1e-12) / np.maximum(mu, 1e-12))
                + (y_adj + 1 / np.maximum(disp, 1e-12))
                * np.log((mu + 1 / np.maximum(disp, 1e-12)) / (y_adj + 1 / np.maximum(disp, 1e-12)))
            )
        r = np.maximum(r, 0)
...
def estimate_glm_robust_disp(
    y,
    design=None,
    prior_df=10,
    update_trend=True,
    trend_method="bin.loess",
    maxit=6,
    k=1.345,
    ...
    if y.get("trended.dispersion") is None:
        y = estimate_glm_trended_disp(y, design=design, method=trend_method, weights=y["weights"])
    if y.get("tagwise.dispersion") is None:
        y = estimate_glm_tagwise_disp(y, design=design, prior_df=prior_df, weights=y["weights"])
    ...
    for i in range(1, int(maxit) + 1):
        ...
        if update_trend:
            ...
            y = estimate_glm_trended_disp(
                y, design=design, method=trend_method, weights=y["weights"]
            )
        ...
        if record:
            y = _record_robust_disp_state(y, i=i, res=res, weights=y["weights"], fit=fit)
