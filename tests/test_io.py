...
class TestReadDGE:
    ...
    def test_read_dge(self):
        dge = ep.read_dge([f"{CSV_DIR}/test_readDGE_s1.txt", f"{CSV_DIR}/test_readDGE_s2.txt"])
...
class TestIORoundTrip:
    ...
    def test_write_read_dge(self):
        ...
        try:
            ...
            assert np.allclose(dge["samples"]["lib.size"].values, y.sum(axis=0))
        finally:
            for fpath in files:
...
class TestReadData:
    ...
    def test_dataframe_input(self):
        df = pd.DataFrame(
            {"S1": [10, 30, 50], "S2": [20, 40, 60]}, index=["gene1", "gene2", "gene3"]
        )
    ...
    def test_csv_input(self):
        ...
        try:
            df = pd.DataFrame({"S1": [10, 30, 50], "S2": [20, 40, 60]}, index=["g1", "g2", "g3"])
            df.to_csv(csv_path)
