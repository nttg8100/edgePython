...
class TestCamera:
    ...
    def test_camera_use_ranks(self, dgelist_disp, design6, gene_sets):
        result = ep.camera(dgelist_disp, gene_sets, design=design6, contrast=1, use_ranks=True)
        assert isinstance(result, pd.DataFrame)

    def test_camera_inter_gene_cor_zero(self, dgelist_disp, design6, gene_sets):
        result = ep.camera(dgelist_disp, gene_sets, design=design6, contrast=1, inter_gene_cor=0)
        assert isinstance(result, pd.DataFrame)

    def test_camera_unsorted(self, dgelist_disp, design6, gene_sets):
        result = ep.camera(dgelist_disp, gene_sets, design=design6, contrast=1, sort=False)
        assert list(result.index) == list(gene_sets.keys())

    def test_camera_sorted_by_pvalue(self, dgelist_disp, design6, gene_sets):
        result = ep.camera(dgelist_disp, gene_sets, design=design6, contrast=1, sort=True)
        assert list(result["PValue"]) == sorted(result["PValue"])

    def test_camera_contrast_vector(self, dgelist_disp, design6, gene_sets):
        result = ep.camera(dgelist_disp, gene_sets, design=design6, contrast=np.array([0.0, 1.0]))
        assert isinstance(result, pd.DataFrame)
...
class TestCameraPart20:
    ...
    def test_camera_fixed_cor(self, d2_camera):
        fit, design, gene_sets = d2_camera
        cam = ep.camera(fit, gene_sets, design=design, contrast=1, inter_gene_cor=0.05)
        assert cam.loc["set3", "Direction"] == "Up"
        assert abs(cam.loc["set3", "PValue"] - 0.2741) < 0.05

    def test_camera_ranks(self, d2_camera):
        fit, design, gene_sets = d2_camera
        cam = ep.camera(fit, gene_sets, design=design, contrast=1, use_ranks=True)
        assert cam.loc["set3", "Direction"] == "Up"
...
class TestFry:
    ...
    def test_fry_single_set(self, dgelist_disp, design6):
        result = ep.fry(dgelist_disp, {"OnlySet": list(range(20))}, design=design6, contrast=1)
        assert len(result) == 1
...
class TestGeneSetStubs:
    ...
    def test_fry_runs(self):
        ...
        d = ep.estimate_disp(d, design)
        gene_sets = {"set1": list(range(0, 20)), "set2": list(range(50, 80))}
...
class TestRoast:
    ...
    def test_roast_returns_dataframe(self, dgelist_disp, design6):
        np.random.seed(42)
        result = ep.roast(dgelist_disp, list(range(10)), design=design6, contrast=1, nrot=99)
        assert isinstance(result, pd.DataFrame)
    ...
    def test_roast_with_dict_index(self, dgelist_disp, design6, gene_sets):
        np.random.seed(42)
        result = ep.roast(dgelist_disp, gene_sets, design=design6, contrast=1, nrot=99)
        assert isinstance(result, pd.DataFrame)
...
class TestMroast:
    ...
    def test_mroast_returns_dataframe(self, dgelist_disp, design6, gene_sets):
        np.random.seed(42)
        result = ep.mroast(dgelist_disp, gene_sets, design=design6, contrast=1, nrot=99)
        assert isinstance(result, pd.DataFrame)
        assert len(result) == len(gene_sets)
...
class TestRomer:
    ...
    def test_romer_returns_dataframe(self, dgelist_disp, design6, gene_sets):
        np.random.seed(42)
        result = ep.romer(dgelist_disp, gene_sets, design=design6, contrast=1, nrot=99)
        assert isinstance(result, pd.DataFrame)
...
class TestRomerStub:
    ...
    def test_romer_stub(self):
        y = np.array([[10, 20, 30], [5, 15, 25], [100, 200, 300]], dtype=float)
        ...
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
            result = ep.romer(fit, {"set1": [0, 1, 2]}, design=design, contrast=1)
        assert isinstance(result, pd.DataFrame)
