...
class TestGoodTuring:
    ...
    def test_large_frequency_table(self):
        ...
        freq1 = np.array([2018046, 449721, 188933, 105668, 68379, 48190, 35709, 37710, 22280])
    ...
    def test_small_counts(self):
        test2 = np.array([312, 14491, 16401, 65124, 129797, 323321, 366051, 368599, 405261, 604962])
...
class TestAddPriorCountPredFC:
    ...
    def test_add_prior_count(self, d1_expr):
        ...
        apc = ep.add_prior_count(d["counts"], lib_size=raw_lib, prior_count=1)
...
class TestSqueezeVar:
    ...
    def test_basic(self, s2_data):
        ...
        assert np.allclose(sv["var_post"][:3], [0.767197, 0.579916, 0.513519], atol=0.01)
    ...
    def test_with_covariate(self, s2_data):
        ...
        assert np.allclose(sv["var_post"][:3], [1.076943, 0.886807, 0.809594], atol=0.01)
    ...
    def test_unequal_df(self, s2_data):
        ...
        assert np.allclose(sv["var_post"][:3], [0.695144, 0.565837, 0.493631], atol=0.01)
...
class TestZscoreQ2Q:
    ...
    def test_zscore_nbinom_2(self):
        z = ep.zscore_nbinom(np.array([0, 1, 5, 10, 20], dtype=float), size=10, mu=8)
    ...
    def test_q2q_nbinom_disp01(self):
        qq = ep.q2q_nbinom(
            np.array([0, 5, 10, 20], dtype=float),
...
class TestSplitIntoGroups:
    ...
    def test_split_into_groups(self):
        y = np.array([[10, 20, 30, 40], [5, 15, 25, 35]], dtype=float)
