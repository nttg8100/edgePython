...
class TestMglmOneWay:
    ...
    def test_fitted_values(self, part1_disp):
        ...
        result = ep.mglm_one_way(part1_disp["counts"][:10, :], design=design, dispersion=0.2)
    ...
    def test_zero_dispersion(self, part1_disp):
        design = np.column_stack([np.ones(4), np.array([0, 0, 1, 1])])
        result = ep.mglm_one_way(part1_disp["counts"][:10, :], design=design, dispersion=0)
...
class TestMglmOneGroup:
    ...
    def test_mglm_one_group(self):
        y = np.array(
            [[10, 20, 30], [5, 15, 25], [100, 200, 300], [0, 0, 0], [50, 50, 50]], dtype=float
        )
    ...
    def test_mglm_one_group_weights(self):
        y = np.array([[10, 20, 30], [5, 15, 25]], dtype=float)
        offset = np.log(np.array([1e6, 1.5e6, 2e6]))
        w = np.array([[1, 1, 2], [1, 1, 2]], dtype=float)
        coefs = ep.mglm_one_group(y, dispersion=0.1, offset=offset, weights=w)
...
class TestGlmFitLRT:
    ...
    def test_glm_lrt_top_gene(self, part1_disp):
        d = part1_disp
        design = np.column_stack([np.ones(4), np.array([0, 0, 1, 1])])
        fit = ep.glm_fit(d, design=design, dispersion=d["common.dispersion"], prior_count=0.5 / 4)
    ...
    def test_glm_fit_coefficients(self, part1_disp):
        d = part1_disp
        design = np.column_stack([np.ones(4), np.array([0, 0, 1, 1])])
        fit = ep.glm_fit(d, design=design, dispersion=d["common.dispersion"], prior_count=0.5)
...
class TestContinuousCovariate:
    ...
    def test_glm_lrt_top_gene(self, part2_data):
        d, y, design = part2_data
        d = ep.calc_norm_factors(d, method="TMM")
        fit = ep.glm_fit(d, design=design, dispersion=0.1, prior_count=0.5 / 3)
        lrt = ep.glm_lrt(fit, coef=1)
        tt = ep.top_tags(lrt)["table"]
...
class TestGlmContrasts:
    ...
    def test_glm_fit_3group_coefficients(self):
        ...
        design = np.column_stack(
            [np.ones(7), (group == 2).astype(float), (group == 3).astype(float)]
        )
        d = ep.make_dgelist(counts=y, group=group)
        fit = ep.glm_fit(d, design=design, dispersion=2 / 3, prior_count=0.5 / 7)
        coef = fit["coefficients"]
...
class TestComplexContrasts:
    ...
    def fit30(self):
        ...
        design = np.column_stack(
            [(grp == "A").astype(float), (grp == "B").astype(float), (grp == "C").astype(float)]
        )
...
class TestGLMWithOffsets:
    ...
    def test_glm_with_offsets(self):
        ...
        offset_mat = np.log(gene_lengths)[:, None] + np.log(lib_sizes)[None, :] - np.log(1e6)
...
class TestLargeScaleRobust:
    ...
    def d41(self):
        ...
        design = np.column_stack([np.ones(6), np.array([0, 0, 0, 1, 1, 1], dtype=float)])
...
class TestContrastTesting:
    ...
    def fits42(self):
        ...
        design = np.column_stack(
            [np.ones(9), (group == 2).astype(float), (group == 3).astype(float)]
        )
        d = ep.make_dgelist(counts=y, group=group)
        d = ep.calc_norm_factors(d)
        d = ep.estimate_disp(d, design)
    ...
    def test_contrast_matches_coef(self, fits42):
        ...
        assert np.allclose(
            lrt_contrast["table"]["LR"].values, lrt_coef["table"]["LR"].values, atol=0.01
        )
