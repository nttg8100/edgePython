...
class TestSpliceVariants:
    ...
    def test_splice_variants(self):
        exon_counts = pd.read_csv(f"{CSV_DIR}/test_data_part22_exons.csv").values.astype(float)
        geneids = np.array([f"Gene{i + 1}" for i in range(5) for _ in range(10)])
...
class TestDiffSpliceDGE:
    ...
    def test_diff_splice_dge(self):
        ...
        exonid = np.array(["e1", "e2", "e3", "e1", "e2", "e1", "e2", "e3"])
        d = ep.make_dgelist(counts=y, group=np.array([1, 1, 1, 2, 2, 2]))
        d = ep.estimate_disp(d, np.column_stack([np.ones(6), [0, 0, 0, 1, 1, 1]]))
        result = ep.diff_splice_dge(d, geneid=geneid, exonid=exonid, dispersion=0.1)
