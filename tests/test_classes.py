...
class TestCompressedMatrix:
    ...
    def test_row(self):
        cm = ep.CompressedMatrix(np.array([[1.0, 2.0, 3.0]]), dims=(4, 3))
        mat = cm.as_matrix()
        assert mat.shape == (4, 3)
        assert np.all(mat == mat[0])

    def test_col(self):
        cm = ep.CompressedMatrix(np.array([[10.0], [20.0], [30.0]]), dims=(3, 5), byrow=False)
        mat = cm.as_matrix()
...
class TestDGEListClass:
    ...
    def test_dgelist(self):
        ...
        samples = pd.DataFrame(
            {"group": pd.Categorical([1, 2]), "lib.size": [90.0, 120.0], "norm.factors": [1.0, 1.0]}
        )
...
class TestDGEExactClass:
    ...
    def test_dge_exact(self):
        table = pd.DataFrame(
            {"logFC": [1.5, -0.5, 0.1], "logCPM": [10.0, 8.0, 12.0], "PValue": [0.001, 0.05, 0.9]},
            index=["gene1", "gene2", "gene3"],
        )
        de = ep.DGEExact({"table": table, "comparison": [1, 2], "genes": None})
...
class TestWLEB:
    ...
    def test_wleb(self):
        ...
        result = ep.WLEB(theta_grid, loglik, prior_n=5, covariate=covariate, trend_method="locfit")
