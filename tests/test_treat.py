...
class TestGlmTreatQL:
    ...
    def test_treat_worst_case_null(self, ql_fit):
        result = ep.glm_treat(ql_fit, coef=1, lfc=np.log2(1.5), null="worst.case")
        assert isinstance(result, dict)
        pv = result["table"]["PValue"].values
        assert np.all(pv >= 0) and np.all(pv <= 1)

    def test_treat_contrast_vector(self, ql_fit):
        result = ep.glm_treat(ql_fit, contrast=np.array([0.0, 1.0]), lfc=np.log2(1.5))
        assert isinstance(result, dict)
...
class TestGlmTreatLRT:
    ...
    def test_treat_lrt_different_lfc(self, lrt_fit):
        ...
        np.testing.assert_array_equal(r1["table"]["logFC"].values, r2["table"]["logFC"].values)

    def test_treat_lrt_worst_case(self, lrt_fit):
        result = ep.glm_treat(lrt_fit, coef=1, lfc=np.log2(1.5), null="worst.case")
        pv = result["table"]["PValue"].values
        assert np.all(pv >= 0) and np.all(pv <= 1)

    def test_treat_lrt_contrast(self, lrt_fit):
        result = ep.glm_treat(lrt_fit, contrast=np.array([0.0, 1.0]), lfc=np.log2(1.5))
        assert len(result["table"]) > 0
