...
def sc_data(rng):
    ...
    sample_ids = np.repeat([f"S{i}" for i in range(1, nsubjects + 1)], cells_per_subject)
    ...
    for g in range(ngenes):
        ...
        subject_effect = np.repeat(rng.normal(0, 0.2, nsubjects), cells_per_subject)
...
class TestGlmScFitValues:
    ...
    def test_no_nan_coefficients(self, twogp_fit):
        ...
        assert not np.any(np.isnan(twogp_fit["coefficients"][conv_mask])), (
...
class TestGlmScTest:
    ...
    def test_output_structure(self, twogp_fit):
        ...
        tab = result["table"]
        for col in ["logFC", "SE", "z", "PValue", "FDR", "sigma_sample", "dispersion", "converged"]:
    ...
    def test_z_equals_logfc_over_se(self, twogp_fit):
        ...
        np.testing.assert_allclose(tab["z"].values, expected_z.values, rtol=1e-10)
    ...
    def test_contrast_sum(self, twogp_fit):
        ...
        np.testing.assert_allclose(tab["logFC"].values, expected_logfc, rtol=1e-10)
...
class TestTopTagsSC:
    ...
    def test_top_tags_with_gene_names(self, sc_data, twogp_design):
        ...
        fit["genes"] = pd.DataFrame({"gene": np.array(gene_names)[gene_mask]})
...
class TestShrinkScDisp:
    ...
    def test_phi_raw_is_reciprocal_of_dispersion(self, twogp_fit):
        ...
        np.testing.assert_allclose(fit["phi_raw"][conv][finite], expected, rtol=1e-10)
    ...
    def test_dispersion_shrunk_is_reciprocal_of_phi_post(self, twogp_fit):
        ...
        np.testing.assert_allclose(fit["dispersion_shrunk"][conv][pos], expected, rtol=1e-10)
    ...
    def test_df_residual_correct(self, twogp_fit, sc_data):
        fit = self._shrink(twogp_fit)
        expected_df = sc_data["ncells"] - twogp_fit["design"].shape[1] - (sc_data["nsubjects"] - 1)
