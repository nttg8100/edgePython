...
bulk_py_times = df[(df["implementation"] == "Python") & (df["pipeline"] == "bulk")][
    "time_seconds"
].values
sc_r_times = df[(df["implementation"] == "R") & (df["pipeline"] == "SC")]["time_seconds"].values
sc_py_times = df[(df["implementation"] == "Python") & (df["pipeline"] == "SC")][
    "time_seconds"
].values

print(
    f"Bulk:  R {bulk_r_times.mean():.2f} +/- {bulk_r_times.std():.2f}s  |  "
    f"Py {bulk_py_times.mean():.2f} +/- {bulk_py_times.std():.2f}s"
)
print(
    f"SC:    R {sc_r_times.mean():.1f} +/- {sc_r_times.std():.1f}s  |  "
...
def jitter_strip(ax, positions, data_list, colors, jitter_width=0.12):
    ...
    for pos, data, color in zip(positions, data_list, colors):
        ...
        ax.hlines(mean_val, pos - 0.2, pos + 0.2, colors=color, linewidths=2.0, zorder=4)


# Panel (a): Bulk
ax = axes[0]
jitter_strip(ax, [0, 1], [bulk_r_times, bulk_py_times], [COLOR_R, COLOR_PY])
ax.set_xticks([0, 1])
...
# Panel (b): Single-cell
ax = axes[1]
jitter_strip(ax, [0, 1], [sc_r_times, sc_py_times], [COLOR_R, COLOR_PY])
ax.set_xticks([0, 1])
...
for ax, letter in zip(axes.flat, "ab"):
    ax.text(-0.15, 1.10, r"\textbf{" + letter + "}", transform=ax.transAxes, fontsize=12, va="top")
