...
ax.scatter(theoretical, observed, c=BLUE, s=2, alpha=0.3, edgecolors="none", rasterized=True)
lims = [
    min(theoretical.min(), observed.min()) * 1.05,
...
ax.scatter(abund_ok, bcv_mle, c=BLUE, s=2, alpha=0.15, edgecolors="none", rasterized=True)
order = np.argsort(abund_ok)
ax.plot(abund_ok[order], bcv_prior[order], color=RED, lw=1.8, label="EB prior trend", zorder=5)
ax.set_yscale("log")
...
ax.scatter(logfc[~sig], logp[~sig], c=GREY, s=2, alpha=0.3, edgecolors="none", rasterized=True)
ax.scatter(logfc[sig], logp[sig], c=RED, s=3, alpha=0.5, edgecolors="none", rasterized=True)
...
for idx in top_idx:
    ...
    ax.annotate(gene, (fc_plot, pval), fontsize=6, alpha=0.8, ha="center", va="bottom")
...
ax.scatter(
    abund_s, bcv_mle_s, c=BLUE_DARK, s=6, alpha=0.3, edgecolors="none", rasterized=True, zorder=2
)
ax.scatter(
    abund_s,
    bcv_post_s,
...
ax.plot(abund_s[order_s], bcv_prior_s[order_s], color=RED, lw=2.0, zorder=5)
...
for ax, letter in zip(axes.flat, "abcd"):
    ax.text(-0.15, 1.08, r"\textbf{" + letter + "}", transform=ax.transAxes, fontsize=12, va="top")
